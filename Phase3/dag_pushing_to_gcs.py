...
GCS_BUCKET = "us-central1-msds697-89149517-bucket"
...
def upload_to_gcs(**kwargs):
    ti = kwargs['ti']
    response = ti.xcom_pull(task_ids='http_test')
    ...
    blob = bucket.blob("movies/movies.json")
    blob.upload_from_string(json.dumps(json_data), content_type="application/json")

    print(f"Uploaded {len(json_data)} records to GCS: movies/movies.json")
